...
<html>
    ...
    <body>
        <%//construindo e enviando request ao endpoint e gerando um JSON Array a partir da response
            ...
            URI uriPets = new URI("http://localhost:8080/Adopet/sample");
            wtPets = client.target(uriPets).path("instituicoes/" + request.getParameter("id") + "/pets");
            Invocation.Builder builderPets = wtPets.request(MediaType.APPLICATION_JSON).header(HttpHeaders.AUTHORIZATION, "Bearer " + "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJhbmRyZXdhbnRzcGV0czEyMyIsImlzcyI6Im15X2FwcCIsImlhdCI6MTY1NzYwMDU3Mn0.3KyezbQ6id46fYKzpkqU-bJ5vz7IdoHpCiuqzo7KQX8");
            Response respostaPets = builderPets.get();
            if (respostaPets.getStatus() != 200) {
                JsonObject respostaMsg = respostaPets.readEntity(JsonObject.class);
            %>
            <jsp:include page="menu.jsp"/>      
        <div class="container mt-4 mb-4">
            <h1 class="text-center mb-4 pb-5"><% out.println(respostaMsg.getString("mensagem"));%></h1><%
            } else {
            JsonObject jsonObjPets = respostaPets.readEntity(JsonObject.class);
            JsonArray jsonArrayPets = jsonObjPets.getJsonArray("pets");
        %>   
        <!-- menu do topo da página -->
        <jsp:include page="menu.jsp"/>      
        <div class="container mt-4 mb-4">
            <h1 class="text-center mb-4 pb-5">Veja os melhores pets para adoção desta instituição!</h1>
        ...
        <%  //início do loop que recupera e renderiza um objeto contido no JSON Array
            for (int i = 0; i < jsonArrayPets.size(); i++) {
                if (!jsonArrayPets.getJsonObject(i).getBoolean("adotado")) {  
                        JsonObject obj = jsonArrayPets.getJsonObject(i);
                ...
                <div class="col">
                    <div class="card h-100 border-success shadow">
                        ...
                        <div class="card-footer text-center">         
                            <a href="desejarPet.jsp?id=<%out.println(obj.getInt("id"));%>" class="btn btn-success">Desejo este Pet!</a>
                        </div>  
                    </div>
                </div>
        <% }} %> 
